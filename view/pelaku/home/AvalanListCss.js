...
export default function AvalanListCss({ navigation }) {
  ...
  const displayDataAvalan = () => HomeController.displayData('daftar-avalan', storedCredentials[0], storedCredentials[14], setListData)
  const checkCsoActive = () => HomeController.checkCsoActive('check-status-cso-avalan', storedCredentials[0], setCsoActive, setTrsId, 'CSS')
    ...
    return (
      <View style={styles.styledContainerMulaiCSO}>
        <Text style={{ fontWeight: 'bold', fontSize: 16, textAlign: 'center' }}>
          Tidak ada CSS Aktif, silahkan tunggu admin memulai CSS terlebih dahulu
        </Text>
      </View>
    )
  } else {
    if (storedCredentials[14] == 0 || storedCredentials[15] != trsId) {
      return (
        <View style={styles.styledContainerMulaiCSO}>
          ...
          <ProcessButton
            ...
            onButtonPressed={() => HomeController.startCSO('mulai-cso-avalan', storedCredentials, 'A', setStoredCredentials, 'CSS')}
        ...
        <View style={styles.styledContainer}>
          <View style={{ flexDirection: 'row', alignItems: 'center' }}>
            <Input
              ...
              value={searchItem}
              setter={search}
              placeHolder='Cari Daftar Avalan CSS'
            />
            <Dropdownitem
          ...
          <ProcessButton
            ...
            onButtonPressed={() => navigation.navigate("TambahAvalan", { trsid: trsId, csoid: storedCredentials[14] })}
          ...
          <ScrollView>
            <Card
              ...
              trsid={trsId}
              csoid={storedCredentials[14]}
            />
          </ScrollView>
          <ProcessButton
            buttonStyle={styles.buttonSubmit}
            onButtonPressed={() => HomeController.submitItem(storedCredentials[14], setListData)}
