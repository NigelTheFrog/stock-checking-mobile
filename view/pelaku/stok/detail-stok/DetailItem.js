...
export default function DetailItem({ route, navigation }) {
    ...
    useEffect(() => {
            ...
            ProcessController.setData(`grade-list?trsdetid=${trsdet}&statusitem=${statusitem}`, "gradecode", "description", setGradeData, 0, setGrade);
    }, []);
                ...
                {itemType == 'R' && statushslcso == 'D' && statussubmit != "P" ?
                    <Dropdownitem
                        ...
                        setter={item => {
                            ...
                            setIsFocus(false);
                            ProcessController.setData(`grade-list?trsdetid=${item.trsdetid}&type=${item['statusitem']}`, "gradecode", "description", setGradeData, 0, setGrade);
                        }}
                    /> :
                    <Input
                        groupStyle={styles.formGroup}
                        ...
                        setter={setItemName}
                        value={itemName}
                        statusEditable={itemType == 'TR' && statushslcso == 'D' && statussubmit != "P" ? true : false}
                        multiline={true}
                    />
                }
